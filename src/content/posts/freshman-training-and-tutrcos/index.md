...
そこでRTOSが提供する、複数のタスクを扱う機能と、タスクと割り込み関数の両方から取り扱うことができるキューやセマフォといった同期プリミティブを用いてこれを解決することができます。たとえば、UARTの受信割り込みがよばれた際、割り込み関数からキューにデータを転送し、メインのタスクではこのキューにデータが来るまでの間待機したり、タイムアウトを設定することができます。このデータを待機している間でも他のタスクや割り込みは問題なく動作するため、制御ループの周期を一定に保つのが容易になるのです。
